...
fib:
    ...
    mov %r8, %r9            ; Copy r8's value into r9, which will hold r8-1
    ...
    mov %r8, %r10           ; Copy r8's value into r10, which will hold r8-2
    ...
    add %r10, %rax          ; Add r10 to rax - rax now contains the value of
                            ; r9 + r10, so we return it here
...
main:
    ...
    jne .errargs            ; If argc != 2, jump to the error message section
    mov %rsi, %r8           ; Move argv into r8
    add $8, %r8             ; Add 8 to r8 to get the pointer to argv[1]
    mov (%r8), %r8          ; Dereference r8 and store the dereferenced value
                            ; in r8
    ...
    mov %r9, %rdx           ; Move fib()'s return value into third argument
                            ; for printf()
